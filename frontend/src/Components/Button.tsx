...
const Button = ({text, icon, style, type, onClick} : ButtonProps) => {
  ...
  return (
    <button 
      className={`rounded-xl ${type == 1 ? type1 : type2} ${style} text-sm  font-bold transition-all delay-[10] flex items-center justify-center`}
      onClick = {onClick}
    >
      {icon} {text}
    </button>
